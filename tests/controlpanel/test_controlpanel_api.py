...
from collective.volto.aemet.testing import RESTAPI_TESTING
...
class SettingsControlpanelTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.controlpanel_url = "/@controlpanels/aemet-settings"
    ...
    def test_controlpanel_listed(self):
        """Test that the AEMET Settings control panel is listed in the control panels overview."""
        response = self.api_session.get("/@controlpanels")

        titles = [x.get("title") for x in response.json()]
        self.assertIn("AEMET Settings", titles)
